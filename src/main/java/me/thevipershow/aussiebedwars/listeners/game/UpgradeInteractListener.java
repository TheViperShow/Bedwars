...
import me.thevipershow.aussiebedwars.game.upgrades.AlarmActiveTrap;
import me.thevipershow.aussiebedwars.game.upgrades.BlindnessPoisonActiveTrap;
...
public final class UpgradeInteractListener extends UnregisterableListener {
    ...
    private static boolean payTraps(final Pair<HashMap<Integer, Integer>, Boolean> result, final Player player, final ShopItem buying, final String boughtName) {
        if (!result.getB()) {
            player.sendMessage(AussieBedwars.PREFIX + "You cannot afford this trap!");
            GameUtils.buyFailSound(player);
            return false;
        } else {
            GameUtils.makePlayerPay(player.getInventory(), buying.getBuyWith(), buying.getBuyCost(), result.getA());
            GameUtils.paySound(player);
            player.sendMessage(AussieBedwars.PREFIX + "You successfully enabled trap: §e" + boughtName);
            return true;
        }
    }
    ...
    private void trapLogic(final Player player, final int clickedSlot) {
        ...
        if (activeTeamTraps.size() >= 0x03) {
            player.sendMessage(AussieBedwars.PREFIX + "You have reached maximum traps limit.");
    ...
    public void onInventoryClick(final InventoryClickEvent event) {
        ...
        final Inventory traps = activeGame.getAssociatedTrapsGUI().get(player);
